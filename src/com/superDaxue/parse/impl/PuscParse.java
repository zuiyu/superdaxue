...
public class PuscParse implements IParse{
/**
 ...
 */
	public List<Courses> parseCourses(String html) {
        ...
        NodeFilter idTable=new HasAttributeFilter("id","dataList");
                    ...
                    for (int k = 0; k < columns.length; k++) {  
                        ...
                        for (int l = 0; l < infoArr.length; l++) {
                        	...
                        	timeTable=new TimeTable();
                       		...
                       		timeTable.setWeek((week+1)+"");
                       		String nameStartStr="style=\"display: none;\" class=\"kbcontent\" >";
                       		String nameStr="<br/><font title='老师'>";
                       		int nameStart=item.indexOf(nameStartStr);
                       		if(nameStart==-1){
                       			nameStartStr="style=\"display: none;\" class=\"kbcontent\">";
                       			nameStart=item.indexOf(nameStartStr);
                       			if(nameStart==-1)
                       				continue;
                       		}
                       		int nameEnd=item.indexOf(nameStr,nameStart);
                       		String name=item.substring(nameStart+nameStartStr.length(), nameEnd);
                       		timeTable.setCourseName(name);
                       		String cycleStr="</font><br/><font title='周次(节次)'>";
                       		int teacherEnd=item.indexOf(cycleStr,nameEnd);
                       		timeTable.setTeacher(item.substring(nameEnd+nameStr.length(),teacherEnd));
                       		String addStr="</font><br/><font title='教室'>";
                       		
                       		int cycleEnd=item.indexOf(addStr,teacherEnd);
                       		if(cycleEnd!=-1){
                       			String cycle=item.substring(teacherEnd+cycleStr.length(),cycleEnd);
	                       		int sd_start=cycle.indexOf("(");
	                       		if(sd_start!=-1){
	                       			timeTable.setSingleDouble(cycle.substring(sd_start-1,sd_start));
	                       			timeTable.setCycle(cycle.substring(0,sd_start));
	                       		}
	                       		else {
									timeTable.setCycle(cycle);
								}
                       			String addEndStr="</font><br/>";
                           		int add_end=item.indexOf(addEndStr,cycleEnd+5);
                           		timeTable.setAddress(item.substring(cycleEnd+addStr.length(),add_end));
                       		}else{
                       			cycleEnd=item.indexOf("</font><br/>",teacherEnd+5);
	                       		String cycle=item.substring(teacherEnd+cycleStr.length(),cycleEnd);
	                       		int sd_start=cycle.indexOf("(");
	                       		if(sd_start!=-1){
	                       			String dsStr=cycle.substring(sd_start,sd_start+1);
	                       			if("单".equals(dsStr)||"双".equals(dsStr)){
	                       				timeTable.setSingleDouble(dsStr);
	                       			}
	                       			timeTable.setCycle(cycle.substring(0,sd_start));
	                       		}
	                       		else {
